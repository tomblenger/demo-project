...
const OrderTable = () => {
  ...
  // Safely get orders data with fallback to empty array
  const ordersData = orders?.data || [];
  
  // Apply filtering logic using useMemo to avoid unnecessary recalculations
  const filteredOrders = useMemo(() => {
    ...
    if (selectVal) {
      filtered = filtered.filter(v => v.status.toLowerCase() === selectVal.toLowerCase());
    }
    
    return filtered;
  }, [orders, searchVal, selectVal]);
  
  // Use pagination with filtered data - this hook is called consistently
  ...
  if (!isLoading && !isError && (!orders?.data || orders.data.length === 0)) {
    content = <ErrorMsg msg="No Orders Found" />;
  }

  if (!isLoading && !isError && orders?.success && orders?.data && orders.data.length > 0) {
    content = (
      <>
            ...
            {currentItems && currentItems.length > 0 ? currentItems.map((item) => (
                ...
                >
                  ...
                  <td className="px-3 py-3 font-normal text-[#55585B] text-end">
                    {item.cart && item.cart.length > 0 ? item.cart.reduce(
                      (acc, curr) => acc + (curr.orderQuantity || 0),
                      0
                    ) : 0}
                  </td>
                  <td className="px-3 py-3 font-normal text-[#55585B] text-end">
        ...
        <div className="flex justify-between items-center flex-wrap">
          <p className="mb-0 text-tiny">
             Showing 1-{currentItems?.length || 0} of {filteredOrders?.length || 0}
          </p>
